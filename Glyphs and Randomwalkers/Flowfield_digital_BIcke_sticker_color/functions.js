...
function draw_gradient_background(c1, c2) {
  ...
  for (let x = 0; x < width; x++) {
    let c = lerpColor(c1, c2, color_gradient_map(x / width));
    console.log(c);
...
function create_noisegrid() {
  ...
  for (var i = 0; i < noise_grid.length; i++) {
    ...
    for (let j = 0; j < noise_grid[i].length; j++) {
      ...
      used_grid[i][j] = false; // subgrid_value;
      ...
      if (
        i == 0 ||
        j == 0 ||
        j == allow_grid[i].length - 1 ||
        i == allow_grid.length - 1
      ) {
        allow_grid[i][j] = 1;
      }
    }
