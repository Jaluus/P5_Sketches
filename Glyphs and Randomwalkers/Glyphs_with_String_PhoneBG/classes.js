class Particle {
  ...
  move() {
    ...
    if (!this.check_move(this.loc) || this.length >= this.max_length) {
      if (this.last_pos.length >= 2) {
  ...
  draw_vertex(current_line_piece, idx, dir) {
    ...
    if (next == undefined) {
      ...
      vertex(
        x_0 + (x_offset * grid_sidelength) / thickness(),
        y_0 + (y_offset * grid_sidelength) / thickness()
      );
      ...
      vertex(
        x_0 + (x_offset_next * grid_sidelength) / thickness(),
        y_0 + (y_offset_next * grid_sidelength) / thickness()
      );
      vertex(
        x_0 + (x_offset * grid_sidelength) / thickness(),
        y_0 + (y_offset * grid_sidelength) / thickness()
